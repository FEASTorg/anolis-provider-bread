...
/**
 * @file session.hpp
 * @brief Serialized CRUMBS session and transport contracts for the BREAD
 * provider.
 */
...
struct SessionStatus {
  ...
  static SessionStatus success() { return {}; }

  /** @brief Construct a failure status with optional native transport details.
   */
  static SessionStatus failure(SessionErrorCode code_in, std::string message_in,
                               int native_code_in = 0, int attempts_in = 0) {
...
public:
  ...
  virtual SessionStatus scan(const ScanOptions &options,
                             std::vector<ScanResult> &out) = 0;
  ...
  virtual SessionStatus read(uint8_t address, RawFrame &frame,
...
public:
   ...
   */
  SessionStatus query_read(uint8_t address, uint8_t reply_opcode,
                           RawFrame &out);
