/**
 * @file session.cpp
 * @brief Serialized CRUMBS session logic layered on top of the transport
 * backend.
 *
...
std::string describe_failure(const std::string &operation,
  ...
  out << operation << " failed" << describe_address(address)
      << " code=" << to_string(status.code) << " attempts=" << status.attempts;
...
SessionStatus
execute_with_retry(const SessionOptions &options, const std::string &operation,
                   const std::optional<uint8_t> &address, Fn &&fn) {
  const int max_attempts = std::max(1, options.retry_count + 1);
  SessionStatus last = SessionStatus::failure(SessionErrorCode::TransportError,
  ...
  for (int attempt = 1; attempt <= max_attempts; ++attempt) {
    ...
    logging::warning(describe_failure(operation, last, address) + " retrying");
  }
...
const SessionOptions &Session::options() const { return options_; }
...
SessionStatus Session::scan(const ScanOptions &options,
...
SessionStatus Session::read(uint8_t address, RawFrame &frame) {
  ...
  const uint32_t timeout_us = options_.timeout_ms > (UINT32_MAX / 1000u)
                                  ? UINT32_MAX
                                  : options_.timeout_ms * 1000u;
...
SessionStatus Session::query_read(uint8_t address, uint8_t reply_opcode,
                                  RawFrame &out) {
  std::lock_guard<std::mutex> lock(mutex_);
...
SessionStatus Session::validate_scan_options(const ScanOptions &options) const {
  if (options.start_address < kMinI2cAddress ||
      options.start_address > kMaxI2cAddress) {
    return SessionStatus::failure(
        SessionErrorCode::InvalidArgument,
        "scan start_address is outside the supported I2C range");
  }
  if (options.end_address < kMinI2cAddress ||
      options.end_address > kMaxI2cAddress) {
    return SessionStatus::failure(
        SessionErrorCode::InvalidArgument,
