/**
 * @file startup.cpp
 * @brief Discovery flow for probing BREAD devices and assembling runtime
 * inventory.
 *
...
inventory::ProbeRecord probe_device(crumbs::Session &session, uint8_t address) {
  ...
  if (bread_parse_version(version_frame.payload.data(),
                          ...
                          &crumbs_ver, &mod_major, &mod_minor,
                          &mod_patch) != 0) {
    probe.status = inventory::ProbeStatus::VersionReadFailed;
  ...
  const inventory::ProbeStatus compat =
      inventory::evaluate_version_compatibility(version_frame.type_id, version,
                                                &compat_detail);
  ...
  if (!caps_status || caps_frame.opcode != BREAD_OP_GET_CAPS) {
    ...
    if (!caps_status) {
      logging::warning("probe " +
                       format_i2c_address(static_cast<int>(address)) +
                       " caps query failed (" + caps_status.message +
                       "), using baseline fallback");
    } else {
      logging::warning(
          "probe " + format_i2c_address(static_cast<int>(address)) +
          " unexpected caps reply opcode " + hex_byte(caps_frame.opcode) +
          ", using baseline fallback");
    }
  } else {
    bread_caps_result_t caps_result{};
    if (bread_caps_parse_payload(
            caps_frame.payload.data(),
  ...
  diag << "probe " << format_i2c_address(static_cast<int>(address))
       ...
       << " crumbs=" << crumbs_ver << " module=" << static_cast<int>(mod_major)
       << "." << static_cast<int>(mod_minor) << "."
       << static_cast<int>(mod_patch) << " caps_source="
       << inventory::to_string(probe.capability_profile.source);
...
DiscoveryResult run_discovery(crumbs::Session &session,
  ...
  if (config.discovery_mode == DiscoveryMode::Scan) {
    ...
    const crumbs::SessionStatus scan_status =
        session.scan(scan_opts, scan_results);
    if (!scan_status) {
      throw std::runtime_error("CRUMBS bus scan failed: " +
                               scan_status.message);
    }
  ...
  if (!build.unsupported_probes.empty()) {
    logging::warning(
        std::to_string(build.unsupported_probes.size()) +
