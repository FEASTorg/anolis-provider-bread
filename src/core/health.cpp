...
/**
 * @file health.cpp
 * @brief Health helpers that project BREAD startup and inventory state into
 * ADPP responses.
 */
...
to_timestamp(const std::chrono::system_clock::time_point &time_point) {
...
ProviderHealth make_provider_health(const runtime::RuntimeState &state) {
  ...
  provider.set_state(!state.ready || degraded ? ProviderHealth::STATE_DEGRADED
  ...
  provider.mutable_metrics()->insert(
      {"device_count", std::to_string(state.devices.size())});
  provider.mutable_metrics()->insert({"ready", state.ready ? "true" : "false"});
  provider.mutable_metrics()->insert({"degraded", degraded ? "true" : "false"});
  provider.mutable_metrics()->insert(
      {"discovery_mode", to_string(state.config.discovery_mode)});
...
make_device_health(const runtime::RuntimeState &state) {
  ...
  for (const auto &device : state.devices) {
    ...
    health.set_state(state.ready ? DeviceHealth::STATE_OK
                                 : DeviceHealth::STATE_UNREACHABLE);
...
void populate_wait_ready(const runtime::RuntimeState &state,
  ...
  response.mutable_diagnostics()->insert(
      {"provider_version", ANOLIS_PROVIDER_BREAD_VERSION});
  response.mutable_diagnostics()->insert(
      {"device_count", std::to_string(state.devices.size())});
  response.mutable_diagnostics()->insert(
  ...
  response.mutable_diagnostics()->insert(
      {"query_delay_us", std::to_string(state.config.query_delay_us)});
  response.mutable_diagnostics()->insert(
      {"unsupported_probe_count",
       std::to_string(state.unsupported_probe_count)});
  response.mutable_diagnostics()->insert(
