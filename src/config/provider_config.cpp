 ...
 * @brief YAML parsing and semantic validation for anolis-provider-bread
 * configuration.
 */
...
ProviderConfig load_config(const std::string &path) {
  ...
  config.discovery_mode = parse_discovery_mode(
  ...
  if (config.discovery_mode == DiscoveryMode::Manual) {
    ...
    if (!addresses_node || !addresses_node.IsSequence() ||
        addresses_node.size() == 0) {
      throw std::runtime_error("discovery.addresses must be a non-empty "
                               "sequence when discovery.mode=manual");
    }

    std::set<int> seen;
    for (std::size_t i = 0; i < addresses_node.size(); ++i) {
      const std::string field_name =
          "discovery.addresses[" + std::to_string(i) + "]";
      const int address = parse_address_value(addresses_node[i], field_name);
      if (!seen.insert(address).second) {
        throw std::runtime_error("Duplicate discovery address: " +
                                 format_i2c_address(address));
      }
      config.manual_addresses.push_back(address);
    }
  } else if (addresses_node && addresses_node.size() > 0) {
    throw std::runtime_error(
        "discovery.addresses is only valid when discovery.mode=manual");
  }
