...
CapabilitySet build_dcmt_capabilities(uint32_t flags) {
    ...
    if((flags & (DCMT_CAP_OPEN_LOOP_CONTROL | DCMT_CAP_CLOSED_LOOP_POSITION | DCMT_CAP_CLOSED_LOOP_SPEED)) != 0u) {
    ...
    if((flags & (DCMT_CAP_CLOSED_LOOP_POSITION | DCMT_CAP_CLOSED_LOOP_SPEED)) != 0u) {
...
CapabilityProfile normalize_capability_profile(DeviceType type, CapabilityProfile profile) {
...
std::optional<std::size_t> find_config_match_by_address(const ProviderConfig &config, int address) {
...
InventoryDevice build_device(const ProviderConfig &config,
    ...
    device.capability_profile = normalize_capability_profile(type, probe.capability_profile);
    ...
    device.descriptor.mutable_tags()->insert({"caps_source", to_string(device.capability_profile.source)});
    device.descriptor.mutable_tags()->insert({"caps_level", std::to_string(device.capability_profile.level)});
    device.descriptor.mutable_tags()->insert({"caps_flags", format_u32_hex(device.capability_profile.flags)});
    device.descriptor.mutable_tags()->insert({"expected", expected ? "true" : "false"});
    device.descriptor.mutable_tags()->insert({"compatibility", "supported"});
    device.descriptor.mutable_tags()->insert({"crumbs_version", format_crumbs_version(device.version.crumbs_version)});
    device.descriptor.mutable_tags()->insert({"module_version", format_module_version(device.version)});

    device.capabilities = build_capabilities(type, device.capability_profile.flags);
    return device;
}
...
CapabilityProfile make_seeded_capability_profile(DeviceType type) {
    ...
    case DeviceType::Dcmt:
        ...
        profile.flags = DCMT_CAP_OPEN_LOOP_CONTROL |
...
InventoryBuildResult build_inventory_from_probes(const ProviderConfig &config,
    ...
    std::sort(sorted_probes.begin(), sorted_probes.end(), [](const ProbeRecord &lhs, const ProbeRecord &rhs) {
        return std::tie(lhs.address, lhs.type_id) < std::tie(rhs.address, rhs.type_id);
    });
    ...
    for(ProbeRecord probe : sorted_probes) {
        ...
        if(match_index.has_value()) {
            ...
            device_id = spec.id;
            label = spec.label.empty() ? generated_label(type, probe.address) : spec.label;
            *type_counter += 1;
        } else {
