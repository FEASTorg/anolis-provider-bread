 ...
 * @brief Version and type compatibility checks for the supported BREAD
 * families.
 */
...
ProbeStatus evaluate_version_compatibility(uint8_t type_id,
  ...
  const int module_rc =
      bread_check_module_compat(version.module_major, version.module_minor,
                                expected.first, expected.second);
  if (module_rc == -1) {
    if (detail) {
      std::ostringstream out;
      out << "module major mismatch: expected "
          << static_cast<int>(expected.first) << " got "
          << static_cast<int>(version.module_major);
      *detail = out.str();
    }
  ...
  if (module_rc == -2) {
    if (detail) {
      std::ostringstream out;
      out << "module minor too old: expected >= "
          << static_cast<int>(expected.second) << " got "
