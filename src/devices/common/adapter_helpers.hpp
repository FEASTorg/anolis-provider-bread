 ...
 * @brief Shared adapter result, payload, and argument helpers for BREAD device
 * adapters.
 */
...
inline bool read_u8(const std::vector<uint8_t> &p, std::size_t off,
                    uint8_t &out) {
  if (off >= p.size())
    return false;
  out = p[off];
...
// Requires p[off] and p[off+1] to be valid. Fails if p.size() < 2 or off+1 >=
// p.size().
inline bool read_u16_le(const std::vector<uint8_t> &p, std::size_t off,
                        uint16_t &out) {
  if (p.size() < 2u || off > p.size() - 2u)
    return false;
  out = static_cast<uint16_t>(p[off]) |
        (static_cast<uint16_t>(p[off + 1u]) << 8u);
  return true;
}

inline bool read_i16_le(const std::vector<uint8_t> &p, std::size_t off,
                        int16_t &out) {
  uint16_t raw = 0u;
  if (!read_u16_le(p, off, raw))
    return false;
...
using ValueMap =
    google::protobuf::Map<std::string, anolis::deviceprovider::v1::Value>;

inline bool get_arg_bool(const ValueMap &args, const std::string &key,
                         bool &out) {
...
inline bool get_arg_int64(const ValueMap &args, const std::string &key,
                          int64_t &out) {
  const auto it = args.find(key);
  if (it == args.end())
    return false;
  if (it->second.type() != anolis::deviceprovider::v1::VALUE_TYPE_INT64)
    return false;
  out = it->second.int64_value();
  return true;
}

inline bool get_arg_uint64(const ValueMap &args, const std::string &key,
                           uint64_t &out) {
  const auto it = args.find(key);
  if (it == args.end())
    return false;
  if (it->second.type() != anolis::deviceprovider::v1::VALUE_TYPE_UINT64)
    return false;
  out = it->second.uint64_value();
  return true;
}

inline bool get_arg_double(const ValueMap &args, const std::string &key,
                           double &out) {
  const auto it = args.find(key);
  if (it == args.end())
    return false;
  if (it->second.type() != anolis::deviceprovider::v1::VALUE_TYPE_DOUBLE)
    return false;
  out = it->second.double_value();
  return true;
}

inline bool get_arg_string(const ValueMap &args, const std::string &key,
                           std::string &out) {
  const auto it = args.find(key);
  if (it == args.end())
    return false;
  if (it->second.type() != anolis::deviceprovider::v1::VALUE_TYPE_STRING)
    return false;
...
inline anolis::deviceprovider::v1::SignalValue
make_signal_value(const std::string &id,
                  const anolis::deviceprovider::v1::Value &val) {
  anolis::deviceprovider::v1::SignalValue sv;
  sv.set_signal_id(id);
  *sv.mutable_value() = val;
  *sv.mutable_timestamp() =
...
/**
 * @brief Map one session-layer read failure into the ADPP error surface used by
 * handlers.
 *
 ...
 * `CODE_UNAVAILABLE`.
 */
inline AdapterReadResult
read_error_from_session(const crumbs::SessionStatus &status,
                        const std::string &op) {
  using S = anolis::deviceprovider::v1::Status;
  if (status.code == crumbs::SessionErrorCode::Timeout) {
    return {false,
            S::CODE_DEADLINE_EXCEEDED,
            op + " timed out: " + status.message,
            {}};
  }
  if (status.code == crumbs::SessionErrorCode::DecodeFailed) {
    return {
        false, S::CODE_INTERNAL, op + " decode error: " + status.message, {}};
  }
  return {false, S::CODE_UNAVAILABLE, op + " failed: " + status.message, {}};
}

/**
 * @brief Map one session-layer call failure into the ADPP error surface used by
 * handlers.
 */
inline AdapterCallResult
call_error_from_session(const crumbs::SessionStatus &status,
                        const std::string &op) {
  using S = anolis::deviceprovider::v1::Status;
  if (status.code == crumbs::SessionErrorCode::Timeout) {
    return {false, S::CODE_DEADLINE_EXCEEDED,
            op + " timed out: " + status.message};
  }
  if (status.code == crumbs::SessionErrorCode::DecodeFailed) {
