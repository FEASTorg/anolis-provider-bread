...
bool parse_state(const std::vector<uint8_t> &payload, RlhtState &out) {
  if (payload.size() < kMinStatePayloadBytes)
    return false;
  if (!read_u8(payload, 0u, out.mode))
    return false;
  if (!read_u8(payload, 1u, out.flags))
    return false;
  if (!read_i16_le(payload, 2u, out.t1_dc))
    return false;
  if (!read_i16_le(payload, 4u, out.t2_dc))
    return false;
  if (!read_i16_le(payload, 6u, out.sp1_dc))
    return false;
  if (!read_i16_le(payload, 8u, out.sp2_dc))
    return false;
  // skip on1_ms[10-11] and on2_ms[12-13]
  if (!read_u16_le(payload, 14u, out.period1))
    return false;
  if (!read_u16_le(payload, 16u, out.period2))
    return false;
  return true;
}
...
AdapterReadResult read_signals(crumbs::Session &session,
  ...
  const crumbs::SessionStatus status = session.query_read(
      static_cast<uint8_t>(device.address), RLHT_OP_GET_STATE, frame);
  ...
  if (should_include(signal_ids, kSigSp1))
    vals.push_back(
        make_signal_value(kSigSp1, make_double_val(s.sp1_dc / 10.0)));
  if (should_include(signal_ids, kSigSp2))
    vals.push_back(
        make_signal_value(kSigSp2, make_double_val(s.sp2_dc / 10.0)));
  ...
  if (should_include(signal_ids, kSigRelay1))
    vals.push_back(make_signal_value(
        kSigRelay1, make_bool_val((s.flags & RLHT_FLAG_RELAY1_ON) != 0u)));
  if (should_include(signal_ids, kSigRelay2))
    vals.push_back(make_signal_value(
        kSigRelay2, make_bool_val((s.flags & RLHT_FLAG_RELAY2_ON) != 0u)));
  if (should_include(signal_ids, kSigEstop))
    vals.push_back(make_signal_value(
        kSigEstop, make_bool_val((s.flags & RLHT_FLAG_ESTOP) != 0u)));
...
AdapterCallResult call(crumbs::Session &session,
  ...
  case 1: { // set_mode
    ...
    append_u8(frame.payload, mode == "open_loop" ? RLHT_MODE_OPEN_LOOP
