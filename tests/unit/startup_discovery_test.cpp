...
public:
  ...
  void add_read_error(uint8_t address, uint8_t opcode,
                      crumbs::SessionErrorCode code) {
...
std::vector<uint8_t> make_version_payload(uint16_t crumbs_ver, uint8_t major,
                                          uint8_t minor, uint8_t patch) {
  return {static_cast<uint8_t>(crumbs_ver & 0xFF),
          static_cast<uint8_t>((crumbs_ver >> 8) & 0xFF), major, minor, patch};
}

// [schema:u8][level:u8][flags:u32_le]
std::vector<uint8_t> make_caps_payload(uint8_t schema, uint8_t level,
                                       uint32_t flags) {
  return {schema,
          level,
...
TEST(StartupDiscoveryTest, ScanModeBuildInventoryFromSupportedRlhtDevice) {
  ...
  transport.add_reply(
      ...
      make_version_payload(CRUMBS_VERSION, RLHT_MODULE_VER_MAJOR,
                           RLHT_MODULE_VER_MINOR, RLHT_MODULE_VER_PATCH));
  transport.add_reply(0x0A, BREAD_OP_GET_CAPS, RLHT_TYPE_ID,
                      make_caps_payload(BREAD_CAPS_SCHEMA_V1, RLHT_CAP_LEVEL_1,
...
TEST(StartupDiscoveryTest, ScanModeDcmtDeviceWithQueriedCaps) {
  ...
  transport.add_reply(
      0x15, 0x00, DCMT_TYPE_ID,
      make_version_payload(CRUMBS_VERSION, DCMT_MODULE_VER_MAJOR,
                           DCMT_MODULE_VER_MINOR, DCMT_MODULE_VER_PATCH));

  const uint32_t level3_flags =
      DCMT_CAP_OPEN_LOOP_CONTROL | DCMT_CAP_BRAKE_CONTROL |
      DCMT_CAP_CLOSED_LOOP_POSITION | DCMT_CAP_CLOSED_LOOP_SPEED |
      DCMT_CAP_PID_TUNING;
  transport.add_reply(
      0x15, BREAD_OP_GET_CAPS, DCMT_TYPE_ID,
      make_caps_payload(BREAD_CAPS_SCHEMA_V1, DCMT_CAP_LEVEL_3, level3_flags));
