...
anolis_provider_bread::ProviderConfig make_stub_config() {
  ...
  config.devices = {
      anolis_provider_bread::DeviceSpec{"rlht0",
                                        anolis_provider_bread::DeviceType::Rlht,
                                        "Left Heater", 0x08},
      anolis_provider_bread::DeviceSpec{"dcmt0",
                                        anolis_provider_bread::DeviceType::Dcmt,
                                        "Conveyor Drive", 0x09},
  };
  return config;
}
...
TEST(
    ProtoSmokeTest,
    DescribeDeviceHandlerProducesSerializableResponseAfterRuntimeInitialization) {
  anolis_provider_bread::runtime::reset();
  anolis_provider_bread::runtime::initialize(make_stub_config());
...
TEST(ProtoSmokeTest, InMemoryProviderLoopCanRoundTripHello) {
  ...
  ASSERT_TRUE(anolis_provider_bread::transport::write_frame(
      outbound, reinterpret_cast<const uint8_t *>(request_payload.data()),
      request_payload.size(), io_error))
      << io_error;
  ...
  ASSERT_TRUE(
      anolis_provider_bread::transport::read_frame(inbound, frame, io_error))
      << io_error;

  anolis::deviceprovider::v1::Request parsed_request;
  ASSERT_TRUE(parsed_request.ParseFromArray(frame.data(),
  ...
  response.mutable_status()->set_code(
      anolis::deviceprovider::v1::Status::CODE_INTERNAL);
  response.mutable_status()->set_message("uninitialized");
  anolis_provider_bread::handlers::handle_hello(parsed_request.hello(),
                                                response);
  ...
  ASSERT_TRUE(anolis_provider_bread::transport::write_frame(
      ...
      response_payload.size(), io_error))
      << io_error;
