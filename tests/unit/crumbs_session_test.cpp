...
public:
  ...
  SessionStatus scan(const ScanOptions &,
  ...
  void delay_us(uint32_t delay_us) override { delays.push_back(delay_us); }
...
TEST(CrumbsSessionTest, QueryReadRetriesWholeOperationOnRetryableFailure) {
  ...
  transport.send_actions.push_back(
      SessionStatus::failure(SessionErrorCode::WriteFailed, "temporary NACK"));
  transport.send_actions.push_back(SessionStatus::success());
  transport.read_actions.push_back(
      {SessionStatus::success(), RawFrame{0x01, 0x80, {0x11, 0x22}}});
...
TEST(CrumbsSessionTest, QueryReadStopsOnNonRetryableFailure) {
  ...
  transport.send_actions.push_back(
      SessionStatus::failure(SessionErrorCode::InvalidArgument, "bad frame"));
