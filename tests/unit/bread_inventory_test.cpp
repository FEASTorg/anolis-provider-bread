...
TEST(BreadInventoryTest, VersionCompatibilityUsesBreadContractRules) {
  ...
  std::string detail;
  EXPECT_EQ(evaluate_version_compatibility(RLHT_TYPE_ID, compatible, &detail),
            ProbeStatus::Supported);
  EXPECT_EQ(detail, "compatible");

  ModuleVersion bad_major = compatible;
  bad_major.module_major = 9;
  EXPECT_EQ(evaluate_version_compatibility(DCMT_TYPE_ID, bad_major, &detail),
            ProbeStatus::IncompatibleModuleMajor);
  ...
  EXPECT_EQ(evaluate_version_compatibility(0x55u, compatible, &detail),
            ProbeStatus::UnsupportedType);
}

TEST(BreadInventoryTest,
     DiscoveryInventoryAssignsStableGeneratedIdsByTypeAndAddress) {
  ...
  const InventoryBuildResult result =
      build_inventory_from_probes(config, probes, InventorySource::Discovered);
  ...
  EXPECT_EQ(result.supported_devices[0].descriptor.tags().at("inventory"),
            "discovered");
  EXPECT_EQ(result.supported_devices[0].descriptor.tags().at("hw.bus_path"),
            "/dev/i2c-1");
  EXPECT_EQ(result.supported_devices[0].descriptor.tags().at("hw.i2c_address"),
            "0x08");
}

TEST(BreadInventoryTest,
     ManualInventoryPreservesConfiguredIdentityAndTracksMissingExpected) {
  ProviderConfig config = make_base_config();
  ...
  const InventoryBuildResult result =
      build_inventory_from_probes(config, probes, InventorySource::Manual);
  ...
  EXPECT_EQ(result.supported_devices[0].descriptor.tags().at("expected"),
            "true");
...
TEST(BreadInventoryTest,
     BaselineFallbackCapsGateDcmtFunctionsAndTypeMismatchIsUnsupported) {
  ProviderConfig config = make_base_config();
  config.discovery_mode = DiscoveryMode::Manual;
  ...
  dcmt_baseline.capability_profile =
      make_baseline_capability_profile(DeviceType::Dcmt);
  ...
  mismatch.capability_profile =
      make_seeded_capability_profile(DeviceType::Dcmt);
  mismatch.detail = "wrong type";

  const InventoryBuildResult result = build_inventory_from_probes(
      config, {dcmt_baseline, mismatch}, InventorySource::Manual);

  ASSERT_EQ(result.supported_devices.size(), 1U);
  EXPECT_EQ(result.supported_devices[0].descriptor.device_id(), "dcmt0");
  EXPECT_EQ(result.supported_devices[0].capability_profile.source,
            CapabilitySource::BaselineFallback);
