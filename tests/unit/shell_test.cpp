...
public:
  ProviderSession(const std::string &exe_path, const std::string &config_path)
      : exe_path_(exe_path), config_path_(config_path) {
#ifdef _WIN32
    ...
    if (!SetHandleInformation(parent_stdin_write, HANDLE_FLAG_INHERIT, 0)) {
      throw std::runtime_error(
          last_error_message("SetHandleInformation(stdin)"));
    }
    ...
    if (!SetHandleInformation(parent_stdout_read, HANDLE_FLAG_INHERIT, 0)) {
      ...
      throw std::runtime_error(
          last_error_message("SetHandleInformation(stdout)"));
    }
    ...
    if (pid_ == 0) {
      ...
      const std::string abs_path =
          std::filesystem::absolute(exe_path_).string();
      execl(abs_path.c_str(), abs_path.c_str(), "--config",
  ...
  anolis::deviceprovider::v1::Response
  send(const anolis::deviceprovider::v1::Request &request) {
    std::string payload;
    ...
    write_all(reinterpret_cast<const uint8_t *>(payload.data()),
              payload.size());
    ...
    if (!response.ParseFromArray(response_payload.data(),
...
private:
  ...
  static std::string last_error_message(const char *context) {
    ...
    FormatMessageA(flags, nullptr, error,
  ...
  void write_all(const uint8_t *data, size_t size) {
    ...
    size_t offset = 0;
    while (offset < size) {
      if (!WriteFile(stdin_write_, data + offset,
  ...
  void read_exact(uint8_t *data, size_t size) {
    ...
    size_t offset = 0;
    while (offset < size) {
      if (!ReadFile(stdout_read_, data + offset,
                    static_cast<DWORD>(size - offset), &read, nullptr)) {
    ...
    while (offset < size) {
      const ssize_t read_count =
          ::read(stdout_fd_, data + offset, size - offset);
      if (read_count <= 0) {
...
TEST(ShellTest, SupportsHelloInventoryAndHealth) {
  ProviderSession session(ANOLIS_PROVIDER_BREAD_TEST_EXE,
                          ANOLIS_PROVIDER_BREAD_TEST_CONFIG);
...

  {
    ...
    ASSERT_EQ(response.status().code(),
              anolis::deviceprovider::v1::Status::CODE_OK);
    EXPECT_EQ(response.wait_ready().diagnostics().at("inventory_mode"),
              "config_seeded");
...

  {
    ...
    EXPECT_EQ(response.get_health().provider().metrics().at("inventory_mode"),
              "config_seeded");
  }
